...
static bool_t phNxpEseProto7816_SendRawFrame(void *conn_ctx, uint32_t data_len, uint8_t *p_data)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s Error phNxpEse_WriteFrame ", __FUNCTION__);
    }
...
static bool_t phNxpEseProto7816_GetRawFrame(void *conn_ctx, uint32_t *data_len, uint8_t **pp_data)
{
    ...
    if (ESESTATUS_SUCCESS != status)
    {
        BoatLog(BOAT_LOG_NORMAL, "%s phNxpEse_read failed , status : 0x%x ", __FUNCTION__, status);
    }
    else
    {
        bStatus = TRUE;
...
static bool_t phNxpEseProto7816_CheckCRC(uint32_t data_len, uint8_t *p_data)
{
    ...
    BoatLog(BOAT_LOG_NORMAL, "Received CRC:0x%x Calculated CRC:0x%x ", recv_crc, calc_crc);
    if (recv_crc != calc_crc)
    {
        status = FALSE;
        BoatLog(BOAT_LOG_NORMAL, "%s CRC failed ", __FUNCTION__);
    }
exit:
    return status;
}
...
static bool_t phNxpEseProto7816_SendSFrame(void *conn_ctx, sFrameInfo_t sFrameData)
{
...
#endif
    default:
        BoatLog(BOAT_LOG_NORMAL, " %s :Invalid S-block", __FUNCTION__);
        return status;
    }
    ...
    BoatLog(BOAT_LOG_NORMAL, "S-Frame PCB: %x ", p_framebuff[PH_PROPTO_7816_PCB_OFFSET]);
...
static bool_t phNxpEseProto7816_sendRframe(void *conn_ctx, rFrameTypes_t rFrameType)
{
    ...
    BoatLog(BOAT_LOG_NORMAL, "%s recv_ack[PH_PROPTO_7816_PCB_OFFSET]:0x%x ", __FUNCTION__, recv_ack[PH_PROPTO_7816_PCB_OFFSET]);
...
static bool_t phNxpEseProto7816_SendIframe(void *conn_ctx, iFrameInfo_t iFrameData)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s Line: [%d] I frame Len is 0, INVALID ", __FUNCTION__, __LINE__);
        return FALSE;
    }
    /* This update is helpful in-case a R-NACK is transmitted from the MW */
...
static bool_t phNxpEseProto7816_SetFirstIframeContxt(void)
{
    ...
    BoatLog(BOAT_LOG_NORMAL, "I-Frame Data Len: %ld Seq. no:%d ", pNextTx_IframeInfo->sendDataLen, pNextTx_IframeInfo->seqNo);
    return TRUE;
}
...
static bool_t phNxpEseProto7816_SetNextIframeContxt(void)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s Process Chained Frame ", __FUNCTION__);
        pNextTx_IframeInfo->isChained = TRUE;
    ...
    BoatLog(BOAT_LOG_NORMAL, "I-Frame Data Len: %ld ", pNextTx_IframeInfo->sendDataLen);
    return TRUE;
}
...
static bool_t phNxpEseProro7816_SaveRxframeData(uint8_t *p_data, uint32_t data_len)
{
    uint32_t offset = 0;
    phNxpEseRx_Cntx_t *pRx_EseCntx = &phNxpEseProto7816_3_Var.phNxpEseRx_Cntx;
    // BoatLog(BOAT_LOG_NORMAL, "Data[0]=0x%x len=%ld Data[%ld]=0x%x Data[%ld]=0x%x ", p_data[0], data_len, data_len - 1, p_data[data_len - 2], p_data[data_len - 1]);
    // BoatLog_hexdump(BOAT_LOG_NORMAL, "p_data : ", p_data, data_len);
    if (pRx_EseCntx->pRsp != NULL)
    {
        offset = pRx_EseCntx->pRsp->len;
        BoatLog(BOAT_LOG_NORMAL, "offset = %d , datalen = %d ", offset, data_len);
        phNxpEse_memcpy((pRx_EseCntx->pRsp->p_data + offset), p_data, data_len);
        pRx_EseCntx->pRsp->len += data_len;
        BoatLog(BOAT_LOG_NORMAL, " phNxpEse_memcpy ok");
        return TRUE;
    }
    else
    {
        BoatLog(BOAT_LOG_NORMAL, "Unsolicited response");
        return FALSE;
    }
}
...
static void phNxpEseProto7816_DecodeSFrameData(uint8_t *p_data)
{
    ...
    while (maxSframeLen > frameOffset)
    {
        ...
        BoatLog(BOAT_LOG_NORMAL, "%s frameoffset=%d value=0x%x ", __FUNCTION__, frameOffset, p_data[frameOffset]);
...
static bool_t phNxpEseProto7816_DecodeFrame(uint8_t *p_data, uint32_t data_len)
{
    ...
    BoatLog(BOAT_LOG_NORMAL, "Retry Counter = %d ", phNxpEseProto7816_3_Var.recoveryCounter);
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s I-Frame Received ", __FUNCTION__);
        ...
        {
            BoatLog(BOAT_LOG_NORMAL, "%s I-Frame lastRcvdIframeInfo.seqNo:0x%x ", __FUNCTION__, pcb_bits.bit7);
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s R-Frame Received", __FUNCTION__);
        phNxpEseProto7816_3_Var.wtx_counter = 0;
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s S-Frame Received ", __FUNCTION__);
        ...
        case WTX_REQ:
            ...
            BoatLog(BOAT_LOG_NORMAL, "%s Wtx_counter value - %lu ", __FUNCTION__, phNxpEseProto7816_3_Var.wtx_counter);
            BoatLog(BOAT_LOG_NORMAL, "%s Wtx_counter wtx_counter_limit - %lu ", __FUNCTION__, phNxpEseProto7816_3_Var.wtx_counter_limit);
            ...
            { /* Checking for WTX counter with max. allowed WTX count */
                if (phNxpEseProto7816_3_Var.wtx_counter == phNxpEseProto7816_3_Var.wtx_counter_limit)
                {
                    ...
                    pNextTx_SframeInfo->sFrameType = INTF_RESET_REQ;
                    phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState = SEND_S_INTF_RST;
                    BoatLog(BOAT_LOG_NORMAL, "%s Interface Reset to eSE wtx count reached!!! ", __FUNCTION__);
#elif defined(T1oI2C_GP1_0)
                    phNxpEseProto7816_3_Var.wtx_counter = 0;
                    pRx_lastRcvdSframeInfo->sFrameType = SWR_REQ;
                    phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.FrameType = SFRAME;
                    pNextTx_SframeInfo->sFrameType = SWR_REQ;
                    phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState = SEND_S_SWR;
                    BoatLog(BOAT_LOG_NORMAL, "%s Software Reset to eSE wtx count reached!!! ", __FUNCTION__);
#endif
                }
                else
                {
        ...
        case INTF_RESET_RSP:
            ...
            if (phNxpEseProto7816_3_Var.recoveryCounter > PH_PROTO_7816_FRAME_RETRY_COUNT)
            {
                ...
                BoatLog(BOAT_LOG_NORMAL, "%s Max retry count reached!!! ", __FUNCTION__);
                phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState = IDLE_STATE;
                status = FALSE;
            }
            else
...
#endif
        default:
            BoatLog(BOAT_LOG_NORMAL, "%s Wrong S-Frame Received ", __FUNCTION__);
            break;
        }
    }
    else
    {
        BoatLog(BOAT_LOG_NORMAL, "%s Wrong-Frame Received ", __FUNCTION__);
    }
exit:
    return status;
}
...
static bool_t phNxpEseProto7816_ProcessResponse(void *conn_ctx)
{
    ...
    if (TRUE == status)
    {
        ...
        {
            BoatLog(BOAT_LOG_NORMAL, "%s CRC Check failed ", __FUNCTION__);
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s phNxpEseProto7816_GetRawFrame failed starting recovery", __FUNCTION__);
            ...
            ((WTX_RSP == pLastTx_SframeInfo->sFrameType) || (RESYNCH_RSP == pLastTx_SframeInfo->sFrameType)))
        {
            ...
            {
                BoatLog(BOAT_LOG_NORMAL, "%s Recovery failed completely, Going to exit ", __FUNCTION__);
                phNxpEseProto7816_3_Var.rnack_retry_counter = PH_PROTO_7816_VALUE_ZERO;
        ...
        else
        {
            ...
            if (phNxpEseProto7816_3_Var.timeoutCounter < PH_PROTO_7816_TIMEOUT_RETRY_COUNT)
            {
                ...
                BoatLog(BOAT_LOG_NORMAL, "%s re-transmitting the previous frame ", __FUNCTION__);
                phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx = phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx;
            }
            else
            {
...
static bool_t TransceiveProcess(void *conn_ctx)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s nextTransceiveState %x ", __FUNCTION__, phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState);
...
#error Either T1oI2C_UM11225 or T1oI2C_GP1_0 must be defined.
        ...
        {
            BoatLog(BOAT_LOG_NORMAL, "%s Transceive send failed, going to recovery! ", __FUNCTION__);
            phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState = IDLE_STATE;
        }
    };
    return status;
...
bool_t phNxpEseProto7816_Transceive(void *conn_ctx, phNxpEse_data *pCmd, phNxpEse_data *pRsp)
{
    ...
    BoatLog(BOAT_LOG_NORMAL, "Enter %s  ", __FUNCTION__);
    ...
    BoatLog_hexdump(BOAT_LOG_NORMAL, "Transceive data ptr ", pCmd->p_data, pCmd->len);
    phNxpEseProto7816_SetFirstIframeContxt();
    status = TransceiveProcess(conn_ctx);
    if (FALSE == status)
    {
        /* ESE hard reset to be done */
        BoatLog(BOAT_LOG_NORMAL, "%s Transceive failed, hard reset to proceed ", __FUNCTION__);
    }
    else if (pRsp->len > reqDataLen)
    {
        BoatLog(BOAT_LOG_NORMAL, "Need '%d' bytes. Got '%d' to copy.", pRsp->len, reqDataLen);
        pRsp->len = 0;
        status = FALSE;
    }
    phNxpEseProto7816_3_Var.phNxpEseProto7816_CurrentState = PH_NXP_ESE_PROTO_7816_IDLE;
...
bool_t phNxpEseProto7816_Open(void *conn_ctx, phNxpEseProto7816InitParam_t initParam, phNxpEse_data *AtrRsp)
{
    ...
    phNxpEseRx_Cntx_t *pRx_EseCntx = &phNxpEseProto7816_3_Var.phNxpEseRx_Cntx;
    status = phNxpEseProto7816_ResetProtoParams();
    BoatLog(BOAT_LOG_NORMAL, "%s: First open completed", __FUNCTION__);
...
bool_t phNxpEseProto7816_Close(void *conn_ctx)
{
    ...
    if (FALSE == status)
    {
        ...
        BoatLog(BOAT_LOG_NORMAL, "%s TransceiveProcess failed  ", __FUNCTION__);
    }
    phNxpEseProto7816_3_Var.phNxpEseProto7816_CurrentState = PH_NXP_ESE_PROTO_7816_IDLE;
    return status;
}
...
bool_t phNxpEseProto7816_IntfReset(void *conn_ctx, phNxpEse_data *AtrRsp)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "%s TransceiveProcess OK  ", __FUNCTION__);
    }
