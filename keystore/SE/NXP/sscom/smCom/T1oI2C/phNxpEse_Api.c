...
ESESTATUS phNxpEse_init(void *conn_ctx, phNxpEse_initParams initParams, phNxpEse_data *AtrRsp)
{
    ...
    if (FALSE == status)
    {
        ...
        BoatLog(BOAT_LOG_NORMAL, "phNxpEseProto7816_Open failed ");
    }
    BoatLog(BOAT_LOG_NORMAL, "phNxpEseProto7816_Open  = %d ", status);
...
ESESTATUS phNxpEse_open(void **conn_ctx, phNxpEse_initParams initParams, const char *pConnString)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, " Session already opened");
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "phPalEse_Init Failed");
...
ESESTATUS phNxpEse_Transceive(void *conn_ctx, phNxpEse_data *pCmd, phNxpEse_data *pRsp)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, " phNxpEse_Transceive - Invalid Parameter no data");
        return ESESTATUS_INVALID_PARAMETER;
    }
    else if ((ESE_STATUS_CLOSE == nxpese_ctxt->EseLibStatus))
    {
        BoatLog(BOAT_LOG_NORMAL, " %s ESE Not Initialized ", __FUNCTION__);
        return ESESTATUS_NOT_INITIALISED;
    }
    else if ((ESE_STATUS_BUSY == nxpese_ctxt->EseLibStatus))
    {
        BoatLog(BOAT_LOG_NORMAL, " %s ESE - BUSY ", __FUNCTION__);
        return ESESTATUS_BUSY;
    }
    else
    {
        ...
        {
            BoatLog(BOAT_LOG_NORMAL, " %s phNxpEseProto7816_Transceive- Failed ", __FUNCTION__);
        }
        ...
        BoatLog(BOAT_LOG_NORMAL, " %s Exit status 0x%x ", __FUNCTION__, status);
        return status;
    }
}
...
ESESTATUS phNxpEse_reset(void *conn_ctx)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "phNxpEse_reset Failed");
    }
    return status;
}
...
ESESTATUS phNxpEse_chipReset(void *conn_ctx)
{
    ...
    {
        status = ESESTATUS_FAILED;
        BoatLog(BOAT_LOG_NORMAL, "phNxpEseProto7816_Reset Failed");
    }
    ...
    if (bStatus != TRUE)
    {
        BoatLog(BOAT_LOG_NORMAL, "phNxpEse_chipReset  Failed");
    }
    return status;
}
...
ESESTATUS phNxpEse_close(void *conn_ctx)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, " %s ESE Not Initialized previously ", __FUNCTION__);
        return ESESTATUS_NOT_INITIALISED;
    }

    phPalEse_i2c_close(nxpese_ctxt->pDevHandle);
    phNxpEse_memset(nxpese_ctxt, 0x00, sizeof(*nxpese_ctxt));
    BoatLog(BOAT_LOG_NORMAL, "phNxpEse_close - ESE Context deinit completed");
...
void phNxpEse_clearReadBuffer(void *conn_ctx)
{
    ...
    BoatLog(BOAT_LOG_NORMAL, "%s Enter ..", __FUNCTION__);
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, "Previous transaction buffer is now cleard");
...
ESESTATUS phNxpEse_read(void *conn_ctx, uint32_t *data_len, uint8_t **pp_data)
{
    ...
    if (ret < 0)
    {
        BoatLog(BOAT_LOG_NORMAL, "PAL Read status error status = %x", status);
        status = ESESTATUS_FAILED;
    }
    else
    {
...
static int phNxpEse_readPacket(void *conn_ctx, void *pDevHandle, uint8_t *pBuffer, int nNbBytesToRead)
{
    ...
    do
    {
        sof_counter++;
        ret = -1;
        BoatSleepMs(ESE_POLL_DELAY_MS);                  /* 1ms delay to give ESE polling delay */
        ret = phPalEse_i2c_read(pDevHandle, pBuffer, 2); /*read NAD PCB byte first*/
        if (ret < 0)
        {
            /*Polling for read on i2c, hence Debug log*/
            BoatLog(BOAT_LOG_NORMAL, "_i2c_read() [HDR]errno : %x ret : %X", errno, ret);
        }
        if (pBuffer[0] == RECIEVE_PACKET_SOF)
        {
            /* Read the HEADR of Two bytes*/
            BoatLog(BOAT_LOG_NORMAL, "%s Read HDR", __FUNCTION__);
        ...
        {
            BoatLog(BOAT_LOG_NORMAL, "%s Recieved NAD byte 0x%x ", __FUNCTION__, pBuffer[0]);
            BoatLog(BOAT_LOG_NORMAL, "%s NAD error, clearing the read buffer ", __FUNCTION__);
        ...
        {
            BoatLog(BOAT_LOG_NORMAL, "%s Chained Pkt, delay read %dms", __FUNCTION__, ESE_POLL_DELAY_MS * CHAINED_PKT_SCALER);
            sm_sleep(ESE_POLL_DELAY_MS);
        }
        else
        {
            BoatLog(BOAT_LOG_NORMAL, "%s Normal Pkt, delay read %dms", __FUNCTION__, ESE_POLL_DELAY_MS * NAD_POLLING_SCALER);
            sm_sleep(ESE_POLL_DELAY_MS);
        }
    } while ((sof_counter < ESE_NAD_POLLING_MAX) && (nxpese_ctxt->EseLibStatus != ESE_STATUS_CLOSE));
    if ((pBuffer[0] == RECIEVE_PACKET_SOF) && (ret > 0))
    {
        BoatLog(BOAT_LOG_NORMAL, "%s SOF FOUND", __FUNCTION__);
        ...
        if ((pBuffer[1] == CHAINED_PACKET_WITHOUTSEQN) || (pBuffer[1] == CHAINED_PACKET_WITHSEQN))
        {
            ...
            BoatLog(BOAT_LOG_NORMAL, "poll_sof_chained_delay value is %d ", poll_sof_chained_delay);
        }
        else
        {
...
ESESTATUS phNxpEse_WriteFrame(void *conn_ctx, uint32_t data_len, const uint8_t *p_data)
{
    ...
    if (nxpese_ctxt->EseLibStatus != ESE_STATUS_CLOSE)
    {
        ...
        if (-1 == dwNoBytesWrRd)
        {
            BoatLog(BOAT_LOG_NORMAL, " - Error in I2C Write.....");
            status = ESESTATUS_FAILED;
        }
        ...
        {
            BoatLog(BOAT_LOG_NORMAL, " I2C Write..... OK");
            status = ESESTATUS_SUCCESS;
...
ESESTATUS phNxpEse_getAtr(void *conn_ctx, phNxpEse_data *pRsp)
{
    ...
    if (status == FALSE)
    {
        BoatLog(BOAT_LOG_NORMAL, "%s Get ATR Failed ", __FUNCTION__);
        return ESESTATUS_FAILED;
    }
    return ESESTATUS_SUCCESS;
}
...
ESESTATUS phNxpEse_getCip(void *conn_ctx, phNxpEse_data *pRsp)
{
    ...
    if (status == FALSE)
    {
        BoatLog(BOAT_LOG_NORMAL, "%s Get CIP Failed ", __FUNCTION__);
        return ESESTATUS_FAILED;
    }
    return ESESTATUS_SUCCESS;
}
