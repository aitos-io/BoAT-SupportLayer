...
U16 smComT1oI2C_Close(void *conn_ctx, U8 mode)
{
    ...
    if (status == ESESTATUS_SUCCESS)
    {
        ...
        if (status != ESESTATUS_SUCCESS)
        {
            BoatLog(BOAT_LOG_NORMAL, "Failed to close ESE interface and free all resources ");
            return SMCOM_COM_FAILED;
        }
    }
    else
    {
        BoatLog(BOAT_LOG_NORMAL, "Failed to close session ");
        return SMCOM_COM_FAILED;
    }
    // }
    // else
    // {
    //     BoatLog(BOAT_LOG_NORMAL, "Invalid conn_ctx");
    // }
    return SMCOM_OK;
}
...
U16 smComT1oI2C_Init(void **conn_ctx, const char *pConnString)
{
    ...
    {
        BoatLog(BOAT_LOG_NORMAL, " Failed to create physical connection with ESE ");
        return SMCOM_COM_FAILED;
    }
    return SMCOM_OK;
}
...
U16 smComT1oI2C_Open(void *conn_ctx, U8 mode, U8 seqCnt, U8 *T1oI2Catr, U16 *T1oI2CatrLen)
{
    ...
    {
        *T1oI2CatrLen = 0;
        BoatLog(BOAT_LOG_NORMAL, " Failed to Open session ");
        return SMCOM_COM_FAILED;
    }
    else
    {
    ...
    BoatLog(BOAT_LOG_NORMAL, " Open session OK");
...
static U32 smComT1oI2C_TransceiveRaw(void *conn_ctx, U8 *pTx, U16 txLen, U8 *pRx, U32 *pRxLen)
{
    ...
    if (txnStatus == ESESTATUS_SUCCESS)
    {
        *pRxLen = pRspTrans.len;
        ...
        BoatLog_hexdump(BOAT_LOG_NORMAL, "APDU Rx<", pRx, pRspTrans.len);
    }
    else
    {
        *pRxLen = 0;
        BoatLog(BOAT_LOG_NORMAL, " Transcive Failed ");
...
U16 smComT1oI2C_AnswerToReset(void *conn_ctx, U8 *T1oI2Catr, U16 *T1oI2CatrLen)
{
    ...
    if (txnStatus == ESESTATUS_SUCCESS)
    {
        *T1oI2CatrLen = pRsp.len;
        ...
        {
            BoatLog(BOAT_LOG_NORMAL, " ATR/CIP Length is improper!!!");
        }
    }
    else
    {
        *T1oI2CatrLen = 0;
        BoatLog(BOAT_LOG_NORMAL, " Failed to Retrieve ATR/CIP status ");
    }
exit:
    return status;
}
...
U16 smComT1oI2C_ComReset(void *conn_ctx)
{
    ...
    if (status != ESESTATUS_SUCCESS)
    {
        BoatLog(BOAT_LOG_NORMAL, "Failed to Reset 7816 protocol instance ");
        return SMCOM_COM_FAILED;
    }
    return SMCOM_OK;
}
