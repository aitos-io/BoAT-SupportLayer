...
BOAT_RESULT boatSemInit(boatSem *semRef,int initialCount)
{
    ...
    semRef->semId = osSemaphoreNew(1,initialCount,NULL);
...
BOAT_RESULT boatSemDestroy(boatSem *semRef)
{
    ...
    status = osSemaphoreDelete(semRef->semId);
    if(status != osOK)
    {
...
BOAT_RESULT boatSemPost(boatSem *semRef)
{
    ...
    status = osSemaphoreRelease(semRef->semId);
    if(status != osOK)
    {
...
BOAT_RESULT boatSemWait(boatSem *semRef,BUINT32 timeout)
{
    ...
    status = osSemaphoreAcquire(semRef->semId,timeout*200/1000);
