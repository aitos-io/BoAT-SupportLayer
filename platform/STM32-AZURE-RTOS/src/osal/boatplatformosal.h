...
struct boatPlatformSem{
    int semid; //! Linux semaphore descriptor
};
#endif
...
struct boatPlatformMutex{
    int mutexid; //! Linux mutex descriptor
};
#endif
...
struct boatPlatformQueue{
    TX_QUEUE queueId; //! Linux message queue descriptor
...
struct boatPlatformTask{
    TX_THREAD taskId; //! Linux thread descriptor
    void *stackPtr; //! stack allocated from task stack pool
};
#endif
...
struct boatPlatformTimer{
    int timerId; //! Linux timer descriptor
