...
BOAT_RESULT boatSemInit(boatSem *semRef,int initialCount)
{
    ...
    {
    	BoatLog(BOAT_LOG_NORMAL, "[boat][sem] boatSemInit err");
		return BOAT_ERROR;
    }
...
BOAT_RESULT boatSemDestroy(boatSem *semRef)
{
	...
	osStatus_t os_status = osSemaphoreDelete(semRef->semId); ///// no return value, void 
	if(osOK != os_status) 
	{
		return BOAT_ERROR;
	}
...
BOAT_RESULT boatSemWait(boatSem *semRef,BUINT32 timeout)
{
	...
	osStatus_t os_status = osSemaphoreAcquire(semRef->semId,timeout);
	if(osOK != os_status) 
	{
		return BOAT_ERROR;
	}
...
BOAT_RESULT boatSemPost(boatSem *semRef)
{
	...
	osStatus_t os_status = osSemaphoreRelease(semRef->semId);
	if(osOK != os_status) 
	{
		return BOAT_ERROR;
	}
