...
void *RpcInit(void)
{
    ...
    rpc_context_ptr = httpClientInit();
...
void RpcDeinit(void *rpc_context_ptr)
{
    ...
    httpClientDeinit(rpc_context_ptr);
}

BOAT_RESULT RpcRequestSet(void *rpc_context_ptr, BCHAR *remote_url_str)
{

    return httpClientSetUrl(rpc_context_ptr, remote_url_str);
}
                           ...
                           BOAT_OUT BUINT32 *response_len_ptr)
{
    ...
    result = httpClientRequestSync(rpc_context_ptr, (const BCHAR *)request_ptr, request_len, (BOAT_OUT BCHAR **)response_pptr, response_len_ptr);
