...
BOAT_RESULT boatI2cMasterWrite(boatI2c *i2cRef, BUINT16 slaveAddr, BUINT32 regAddr, BUINT8 *data, BUINT16 dataLen)
{
    ...
    if (slaveAddr > 0x7f)
    {
        BoatLog(BOAT_LOG_NORMAL, "[boat][I2C] L610 i2c writing slave_addr is invalid!\r\n");
        return BOAT_ERROR;
    }
    if (regAddr > 0xffff)
    {
        BoatLog(BOAT_LOG_NORMAL, "[boat][I2C] L610 i2c writing slave device reg_addr is invalid!\r\n");
        return BOAT_ERROR;
    }
    if (data == NULL)
    {
...
BOAT_RESULT boatI2cMasterRead(boatI2c *i2cRef, BUINT16 slaveAddr, BUINT32 regAddr, BUINT8 *data, BUINT16 dataLen)
{
    ...
    if (slaveAddr > 0x7f)
    {
        BoatLog(BOAT_LOG_NORMAL, "[boat][I2C] L610 i2c reading slave_addr is invalid!\r\n");
        return BOAT_ERROR;
    }
    if (regAddr > 0xffff)
    {
        BoatLog(BOAT_LOG_NORMAL, "[boat][I2C] L610 i2c reading slave device reg_addr is invalid!\r\n");
        return BOAT_ERROR;
    }
    if (data == NULL)
    {
