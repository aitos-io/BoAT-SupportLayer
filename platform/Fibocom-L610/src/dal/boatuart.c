...
#define UART_BUF_LEN_MAX 10*1024
...
BOAT_RESULT boatUartCbRegister(hal_uart_port_t port, boatUart *uartRef, boatUartRxCallback rxCallback)
{
	if(uartRef == NULL)
	{
		BoatPrintf(0,"[boat][uart] L610 uart device uartRef is NULL!\r\n");
		return BOAT_ERROR;
	}
	if(rxCallback == NULL)
	{
		BoatPrintf(0,"[boat][uart] L610 uart device rxCallback is NULL!\r\n");
		return BOAT_ERROR;
	}
	...
	for(int i=0; i< FIBO_UART_MAX; i++)
	{
		if((boatFiboUartRxCbInfo[i].callback == NULL) && (boatFiboUartRxCbInfo[i].uartRef == NULL))
		{
			...
			BoatPrintf(0,"[boat][uart] L610 uart rxCallback regitster succ!\r\n");
			return BOAT_SUCCESS;
		}
	}
	BoatPrintf(0,"[boat][uart] L610 uart rxCallback regitster full!\r\n");
	return BOAT_ERROR;
}
...
BOAT_RESULT boatUartCbDeregister(hal_uart_port_t port, boatUart *uartRef)
{
	if(uartRef == NULL)
	{
		BoatPrintf(0,"[boat][uart] L610 uart device deregister uartRef is NULL!\r\n");
		return BOAT_ERROR;
	}
	if(port < 0)
	{
		BoatPrintf(0,"[boat][uart] L610 uart device deregister rxCallback is NULL!\r\n");
		return BOAT_ERROR;
	}
	...
	for(int i=0; i< FIBO_UART_MAX; i++)
	{
		if((boatFiboUartRxCbInfo[i].port == port) && (boatFiboUartRxCbInfo[i].uartRef == uartRef))
		{
			...
			BoatPrintf(0,"[boat][uart] L610 uart rxCallback deregitster succ, port[%d]!\r\n", port);
			return BOAT_SUCCESS;
		}
	}
	BoatPrintf(0,"[boat][uart] L610 uart rxCallback deregitster not fount the port[%d]!\r\n", port);
	return BOAT_ERROR;
}
...
void fiboUartCommCallback(hal_uart_port_t port, UINT8 *data, UINT16 len, void *arg)
{
	...
	{
		BoatPrintf(0,"[boat][uart] L610 uart callback port number invalid\r\n");
		return;
	}
	if(data == 0)
	{
		BoatPrintf(0,"[boat][uart] L610 uart callback *data invalid\r\n");
		return;
	}
	for(int i = 0; i < FIBO_UART_MAX; i++)
	{
...
BOAT_RESULT boatUartInit(boatUart *uartRef, BUINT8 port, boatUartConfig config, boatUartRxCallback rxCallback)
{
	...
	{
		BoatPrintf(0,"[boat][uart] L610 uart port is invalid!\r\n");
		return BOAT_ERROR;
	}
	//! baudrate : Not be limited, it can be any value. 0 set as auto baud
	/////if(config.baudrate)	
	{
		BoatPrintf(0,"[boat][uart] L610 uart config.baudrate is[%d!\r\n",config.baudrate);
		/////return BOAT_ERROR;
	}
	//! databit : 5\6\7\8
	if((config.databit < UART_DATABITS_MIN)	 || (config.databit > UART_DATABITS_MAX))
	{
		BoatPrintf(0,"[boat][uart] L610 uart config.databit is invalid!\r\n");
		return BOAT_ERROR;
	}
	//! stopbit : 1/2
	if((config.stopbit > UART_STOPBITS_MAX)	|| (config.stopbit < UART_STOPBITS_MIN)) 
	{
		BoatPrintf(0,"[boat][uart] L610 uart config.stopbit is invalid!\r\n");
		return BOAT_ERROR;
	}
 	//! parity 	 0: No parity check, 1: Parity check is odd, 2: Parity check is even
	if(config.parity > UART_PARITY)	
	{
		BoatPrintf(0,"[boat][uart] L610 uart config.parity is invalid!\r\n");
		return BOAT_ERROR;
	}
	//! rxBufSize; 
	if(config.rxBufSize > UART_BUF_LEN_MAX)	
	{
		BoatPrintf(0,"[boat][uart] L610 uart config.rxBufSize over the max\r\n");
		return BOAT_ERROR;
	}
	if(config.rxBufSize == 0)	
	{
		BoatPrintf(0,"[boat][uart] L610 uart config.rxBufSize equal 0, set as default[%d] \r\n",UART_BUF_LEN_DEFAULT);
		config.rxBufSize = UART_BUF_LEN_DEFAULT;
	}

	//! txBufSize 
	if(config.txBufSize > UART_BUF_LEN_MAX)	
	{
		BoatPrintf(0,"[boat][uart] L610 uart config.txBufSize over the max\r\n");
		return BOAT_ERROR;
	}
	if(config.txBufSize == 0)	
	{
		BoatPrintf(0,"[boat][uart] L610 uart config.txBufSize equal 0, set as default[%d] \r\n",UART_BUF_LEN_DEFAULT);
	...
	if(ret == BOAT_ERROR)
	{
        BoatPrintf(0, "[boat][uart] L610 uart regitster callback full, port[%d].",port);
		return BOAT_ERROR;
	}
	...
	if (ret < 0)
    {
        BoatPrintf(0, "[boat][uart] L610 Initialize the uart[%d] failed.", port);
...
BOAT_RESULT boatUartWrite(boatUart *uartRef, unsigned char *data, BUINT32 len)
{
	...
	if(uartRef == NULL)
	{
		BoatPrintf(0,"[boat][uart] L610 uart write uartRef is NULL!\r\n");
		return BOAT_ERROR;
	}

	if(uartRef->uartPort < 0)	//! > UART_PORT_NUMBER_MAX)	
	{
		BoatPrintf(0,"[boat][uart] L610 uart port is invalid][%d]\r\n",uartRef->uartPort);
		return BOAT_ERROR;
	}
	...
	if(data == NULL)
	{
		BoatPrintf(0,"[boat][uart] L610 uart write data is NULL!\r\n");
		return BOAT_ERROR;
	}
	...
	if(len == 0)
	{
		BoatPrintf(0,"[boat][uart] L610 uart write len equal 0, return succ!\r\n");
		return BOAT_SUCCESS;
	}
	...
	if (ret < 0)
    {
        BoatPrintf(0, "[boat][uart] L610uart[%d] put(write) failed.", uartRef->uartPort);
...
BOAT_RESULT boatUartDeinit(boatUart *uartRef)
{
	...
	if (ret < 0)
    {
        BoatPrintf(0, "[boat][uart] L610uart[%d] deinit failed.", uartRef->uartPort);
    }
