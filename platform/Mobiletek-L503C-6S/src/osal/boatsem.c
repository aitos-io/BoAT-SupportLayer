...
BOAT_RESULT boatSemInit(boatSem *semRef,int initialCount)
{
    ...
    os_status = ol_os_sem_creat(&(semRef->semId),initialCount,MBTK_OS_FIFO);
	if(mbtk_os_success != os_status)  
    {
    	semRef->semId = NULL;
...
BOAT_RESULT boatSemDestroy(boatSem *semRef)
{
	...
	mbtk_os_status os_status = ol_os_sem_delete(semRef->semId); ///// no return value, void 
	if(mbtk_os_success != os_status) 
	{
		return BOAT_ERROR;
	}
...
BOAT_RESULT boatSemWait(boatSem *semRef,BUINT32 timeout)
{
	...
	mbtk_os_status os_status = ol_os_sem_request(semRef->semId,timeout);
	if(mbtk_os_success != os_status) 
	{
		return BOAT_ERROR;
	}
...
BOAT_RESULT boatSemPost(boatSem *semRef)
{
	...
	mbtk_os_status os_status = ol_os_sem_release(semRef->semId);
	if(mbtk_os_success != os_status) 
	{
		return BOAT_ERROR;
	}
