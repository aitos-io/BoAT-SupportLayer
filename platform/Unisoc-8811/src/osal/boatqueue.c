...
BOAT_RESULT boatQueueInit(boatQueue *msgQRef, char *queueName, BUINT32 maxSize, BUINT32 maxNumber)
{
    ...
    if (queueName == NULL)
    {
        msgQRef->queueId = osiMessageQueueCreate(maxNumber, maxSize);
        msgQRef->name = queueName;
    }
...
BOAT_RESULT boatQueueDelete(boatQueue *msgQRef)
{
    ...
    osiMessageQueueDelete(msgQRef->queueId);
...
BOAT_RESULT boatQueueSend(const boatQueue *msgQRef, BUINT8 *msgPtr, BUINT32 msgLen, BUINT32 timeout)
{
    ...
    if (timeout > 0)
    {
        ret = osiMessageQueueTryPut(msgQRef->queueId, msgPtr, timeout);
    }
    else
    {
        ret = osiMessageQueuePut(msgQRef->queueId, msgPtr);
    }
...
BOAT_RESULT boatQueueReceive(const boatQueue *msgQRef, BUINT8 *msgPtr, BUINT32 msgLen, BUINT32 timeout)
{
    ...
    if (timeout > 0)
    {
        ret = osiMessageQueueTryGet(msgQRef->queueId, msgPtr, timeout);
    }
    else
    {
        ret = osiMessageQueueGet(msgQRef->queueId, msgPtr);
    }
