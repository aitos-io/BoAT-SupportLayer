...
BOAT_RESULT boatSemInit(boatSem *semRef,int initialCount)
{
    ...
    if((initialCount != 0) && \
        (initialCount != 1))
    {
        BoatLog(BOAT_LOG_CRITICAL,"Only binary semaphore are supported");
        return BOAT_ERROR_COMMON_INVALID_ARGUMENT;
    }

    QlOSStatus ret = ql_rtos_semaphore_create(&semRef->semId, initialCount);
    if(ret != BOAT_SUCCESS)
    {
...
BOAT_RESULT boatSemDestroy(boatSem *semRef)
{
    ...
    ret = ql_rtos_semaphore_delete(semRef->semId);
    if(ret != BOAT_SUCCESS)
    {
        BoatLog(BOAT_LOG_CRITICAL,"Sem delete failed!");
        return BOAT_ERROR;
    }
...
BOAT_RESULT boatSemPost(boatSem *semRef)
{
    ...
    ret = ql_rtos_semaphore_release(semRef->semId);
    if(ret != BOAT_SUCCESS)
    {
        BoatLog(BOAT_LOG_CRITICAL,"Sem post failed!");
        return BOAT_ERROR;
    }
...
BOAT_RESULT boatSemWait(boatSem *semRef,BUINT32 timeout)
{
    ...
    ret = ql_rtos_semaphore_wait(semRef->semId,timeout);
