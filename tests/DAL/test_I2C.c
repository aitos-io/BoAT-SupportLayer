...
void testI2CConfigSet(BoATI2CConfig_t *config, BUINT32 speed, BUINT8 SlavAddrBits, BUINT8 SlaveRegAddrLen)
{
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0050_boatI2CMasterWrite_Successful(void)
{
	...
	if(rtnVal == BOAT_SUCCESS)
	{
		rtnVal = boatI2CMasterWrite(&testI2CID,testSlaveAddr,0,td,sizeof(td));//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
		boatAssert_int_eq(rtnVal,0);
	}
	boatI2CClose(&testI2CID);///// must destroy the timer once you stop using it.
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0051_boatI2CMasterWrite_Successful_DataLenZero(void)
{
	...
	if(rtnVal == BOAT_SUCCESS)
	{
		rtnVal = boatI2CMasterWrite(&testI2CID,testSlaveAddr,0,td,0);//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
		boatAssert_int_eq(rtnVal,0);
	}
	boatI2CClose(&testI2CID);///// must destroy the timer once you stop using it.
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0052_boatI2CMasterWrite_Failed_I2CRefNULL(void)
{
	...
	rtnVal = boatI2CMasterWrite(0,testSlaveAddr,0,td,sizeof(td));//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
	boatAssert_int_eq(rtnVal,-1);
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0053_boatI2CMasterWrite_Failed_SlaveAddrInvalid(void)//! slave address > 0x7f is invalid
{
	...
	if(rtnValOpen == BOAT_SUCCESS)
	{
		...
		{
			if((i < 0x80)) continue;
			rtnVal = boatI2CMasterWrite(&testI2CID,i,0,td,sizeof(td));//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
			if(rtnVal == BOAT_SUCCESS)
			{
				break;
			}
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0054_boatI2CMasterWrite_Failed_RegAddrInvalid(void)//! slave address > 0x7f is invalid
{
	...
	if(rtnValOpen == BOAT_SUCCESS)
	{
		for(BUINT32 i = 0;;i++)
		{
			...
			rtnVal = boatI2CMasterWrite(&testI2CID,testSlaveAddr,i,td,sizeof(td));//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
			if(rtnVal == BOAT_SUCCESS)
			{
				break;
			}
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0055_boatI2CMasterWrite_Failed_DataNULL(void)//! slave address > 0x7f is invalid
{
	...
	if(rtnValOpen == BOAT_SUCCESS)
	{
		rtnVal = boatI2CMasterWrite(&testI2CID,testSlaveAddr,0,NULL,8);//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
	}
	boatAssert_int_eq(rtnVal,-1);
	boatI2CClose(&testI2CID);///// must destroy the timer once you stop using it.
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0056_boatI2CMasterWrite_Failed_DataLenInvalid(void)//! slave address > 0x7f is invalid
{
	...
	if(rtnValOpen == BOAT_SUCCESS)
	{
		...
		{
			if((i < 9)) continue;
			rtnVal = boatI2CMasterWrite(&testI2CID,testSlaveAddr,0,td,i);//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
			if(rtnVal == BOAT_SUCCESS)
			{
				break;
			}
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0058_boatI2CMasterRead_Successful(void)
{
	...
	if(rtnVal == BOAT_SUCCESS)
	{
		rtnVal = boatI2CMasterRead(&testI2CID,testSlaveAddr,0,rd,sizeof(rd));//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
		boatAssert_int_eq(rtnVal,0);
	}
	boatI2CClose(&testI2CID);///// must destroy the timer once you stop using it.
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0059_boatI2CMasterRead_Successful_DataLenZero(void)
{
	...
	if(rtnVal == BOAT_SUCCESS)
	{
		rtnVal = boatI2CMasterRead(&testI2CID,testSlaveAddr,0,rd,0);//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
		boatAssert_int_eq(rtnVal,0);
	}
	boatI2CClose(&testI2CID);///// must destroy the timer once you stop using it.
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0060_boatI2CMasterRead_Failed_I2CRefNULL(void)
{
	...
	rtnVal = boatI2CMasterRead(0,testSlaveAddr,0,rd,sizeof(rd));//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
	boatAssert_int_eq(rtnVal,-1);
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0061_boatI2CMasterRead_Failed_SlaveAddrInvalid(void)//! slave address > 0x7f is invalid
{
	...
	if(rtnValOpen == BOAT_SUCCESS)
	{
		...
		{
			if((i < 0x80)) continue;
			rtnVal = boatI2CMasterRead(&testI2CID,i,0,rd,sizeof(rd));//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
			if(rtnVal == BOAT_SUCCESS)
			{
				break;
			}
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0062_boatI2CMasterRead_Failed_RegAddrInvalid(void)//! slave address > 0x7f is invalid
{
	...
	if(rtnValOpen == BOAT_SUCCESS)
	{
		...
		{
			if((i < 0x10000)) continue;
			rtnVal = boatI2CMasterRead(&testI2CID,testSlaveAddr,i,rd,sizeof(rd));//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
			if(rtnVal == BOAT_SUCCESS)
			{
				break;
			}
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0063_boatI2CMasterRead_Failed_DataNULL(void)//! slave address > 0x7f is invalid
{
	...
	if(rtnValOpen == BOAT_SUCCESS)
	{
		rtnVal = boatI2CMasterRead(&testI2CID,testSlaveAddr,0,NULL,8);//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
	}
	boatAssert_int_eq(rtnVal,-1);
	boatI2CClose(&testI2CID);///// must destroy the timer once you stop using it.
...
void *test_BoAT_DAL_FibocomL610_01I2C_test_0064_boatI2CMasterRead_Failed_DataLenInvalid(void)//! slave address > 0x7f is invalid
{
	...
	if(rtnValOpen == BOAT_SUCCESS)
	{
		...
		{
			if((i < 9)) continue;
			rtnVal = boatI2CMasterRead(&testI2CID,testSlaveAddr,0,rd,i);//!(boatI2C *I2CRef, BUINT16 slave_addr, BUINT32 reg_addr, BUINT8 *data, BUINT16 datalen)
			if(rtnVal == BOAT_SUCCESS)
			{
				break;
			}
